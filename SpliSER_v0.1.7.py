...
from Gene_Site_Iter_Graph_v017 import Gene, Site, Iter, Graph
import numpy
...
def binary_site_search(array, pos, strand):
	'''
	...
	while stuck == False and found == False :
		...
		if int(pos) == int(array[idx].getPos()): #if the site position matches
			if strand == array[idx].getStrand(): #if the strand also matches
				found = True
				break
				...
				for a in [idx-1,idx+1]: #look up and downstream, then break the loop.
					if a >=0 and a <len(array) and int(pos) == int(array[a].getPos()) and strand == array[a].getStrand():
						idx=a
						found=True
				break 
			...
			if qChrom == chrom or qChrom == "All":
				...
				if qGene =="All" or LinGene or RinGene:
					...
					if len(site2D_array[chrom_idx])>0:
						left_site_idx = binary_site_search(site2D_array[chrom_idx],leftpos,strand)
						right_site_idx = binary_site_search(site2D_array[chrom_idx],rightpos,strand)
				...
				if iterGo[idx] == True:
					...
					if nextLine is not None:
						...
						if qGene == 'All': #If we are assessing all sites - proceed as normal
							...
							iterGo[idx] = False
						else: #If we are looking for sites form a particular gene
							qGeneFound = False
			...
			if chroms[idx] == currentChrom:
				...
				if pos < lowestPos or lowestPos == -1 or (isStranded==True and pos==lowestPos and strand=="+"):
					...
					if reads >= minReads and sse >= minSSE:
						...
						posCounter = 0
				#else if lowestPos is seen again, count up how many times it is seen with more than 'minReads' reads
				elif pos == lowestPos:
					reads = int(currentVals[idx][5])+ int(currentVals[idx][6])+int(currentVals[idx][7])
					if reads >= minReads and sse >= minSSE:
