...
import bisect
import HTSeq
from spliser.Gene_Site_Iter_Graph_v1 import Gene
...
def createGenes(annotation, aType, qGene):
	...
	#Find the maximum gene RH boundary seen so far to the left of each gene (this is used for assigning sites to genes)
	for gene_array in gene2D_array:
		max_right = -1
		#print(len(gene_array))
		
		for gene in gene_array:
			max_right = max(max_right, int(gene.getRightPos()))
			gene.setMaxRightBefore(max_right)
