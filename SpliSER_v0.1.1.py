"""
SpliSER- Splice-site Strength Estimate from RNA-seq
...
from Gene_Site_Iter import Gene, Site, Iter
import numpy
...
def createGenes(annotation, sampleNum, qGene):
	"""
	...
	----------
	annotation : String
		The absolute file path of a GFF3 annoation file
	sampleNum : int
		The number of samples that will eventually be recorded in each Gene object.
	qChrom : String
	...
	"""
	geneCounter = 0
	for lineIDX, line in enumerate(open(annotation,'r')):
		if lineIDX >0:
			...
			GeneLeft = values[2]
			GeneRight = values[3]
			GeneName = str(values[4]).rstrip()#FOr ENSEMBL GENES
			strand = str(values[1])
...
def binary_gene_search(array, pos):
	'''
	...
	while stuck == False and found == False:
		if int(pos) >= int(array[idx].getLeftPos()) and int(pos) <= int(array[idx].getRightPos()) : #if one site in junction lie within the given gene
			found = True
			break
...
def findAlphaCounts(bedFile, qChrom, qGene, maxIntronSize, sample=0, numsamples=1):
	"""
				...
				if qGene =="All" or LinGene or RinGene:
					...
					for num, site_info in enumerate([[leftpos,left_site_idx],[rightpos,right_site_idx]]):
						...
						if site_info[1] == -1:
							...
							gene_idx = binary_gene_search(gene2D_array[chrom_idx], site_info[0])
...
def checkBam(bedFile, sSite, sample):
				...
				for idx, d in enumerate(digits):
					...
					if case in ['N']:
						...
						if rSite == targetPos:
							partnerUsed = lSite
							alpha_read = True
						#check if this is a partnership between a known partner and competitor
						#if lSite in partners or lSite in competitors:
...
def outputData(outputFile, qChrom, minReads, inGene, Titles):
        ...
        for c_index, c in enumerate(chrom_index):
                for site in site2D_array[c_index]:
                        if inGene == "All" or inGene == site.getGeneName():
                                out = open(outputFile+"."+str(inGene)+"_"+str(site.getPos())+".tsv","w+") #open a file named after the splice site
                                for fileIndex, t in enumerate(Titles):
                                        if site.getAlphaCount(fileIndex)+site.getBeta1Count(fileIndex)+site.getBeta2Count(fileIndex) >= minReads:
                                                out.write(str(t.split("_")[0])+"\t"+str(t.split("_")[1])+"\t"+str(t.split("_")[2])+"\t"+str(site.getSE(fileIndex))+"\n")
                                        else:
...
def processSites(inBAM, qChrom, sample = 0, numsamples = 1):
	print('Processing sample '+ str(int(sample)+1)+' out of '+str(numsamples))
...
def process(inBAM, inBed, outputPath, threads, qGene, qChrom, maxIntronSize, annotationFile):
	print('Processing')

	if annotationFile is not None:
		print('\n\nStep 0: Creating Genes from Annotation...')
		createGenes(annotationFile,1, qGene)
	...
	findAlphaCounts(inBed,qChrom, qGene, int(maxIntronSize)) #We apply theqGene filter here, where the splice site objects are made

	print('\n\nStep 2: Identifying Competitors of each splice site...')
	findCompetitorPos()

	print('\n\nStep 3: Finding Beta reads')
	processSites(inBAM,qChrom)
...
def combine(samplesFile, outputPath,firstChrom):
	...
	currentChrom = firstChrom # The chromosome currently being assessed
	lowestPos = -1
	...
	count = 0
	filledCount = 0
	print('updated currentChrom to {}'.format(firstChrom))
	...
	while not(len(set(iterDone)) <=1 and iterDone[0] ==True): #Stop if all files are done iterating
		...
		if not(len(set(iterDone)) <=1 and iterDone[0] ==True):
			...
			#if all bed files have arrived at a chromosome, set it as the current chromosome - otherwise do not change
			chromsCheck = [i for i in chroms if i is not None] # list comprehension away 'None' values
			if checkEqual2(chromsCheck) and currentChrom != chromsCheck[0] : # if all values are the same, and this is a new value
				currentChrom = chromsCheck[0]
				count = 0 # reset Site Counter
				...
				for idx, vals in enumerate(currentVals):
						if vals[0] == currentChrom and int(vals[1]) == lowestPos and iterDone[idx] ==False: #if this sample has values for the splice site
							...
							cPosList = literal_eval(str(vals[10]))
							for c in cPosList:
				...
				for idx, vals in enumerate(currentVals):
						if vals[0] == currentChrom and vals[1] == lowestPos and iterDone[idx] ==False: #if this sample has values for the splice site
							...
							if qGene == 'All' or qGene == assocGene:
								...
								print('dont need to check bam', 'site', count, 'file',idx)
				#output lines for this splice site
				if qGene == 'All' or qGene == assocGene:
					outputCombinedLines(outTSV,sSite, assocGene)
...
def diffSpliSE_output(samplesFile,combinedFile, outputPath, minReads, qGene):
	...
	outDiff.write("Region\tSite\tGene")
	for t in allTitles:
...
if __name__ == "__main__":
	...
	parser_process.add_argument('-O', '--outputPath', dest='outputPath', help="Absolute path, including file prefix where SpliSER will write the .SpliSER.bed file", required=True)
	...
	parser_process.add_argument('-A', '--annotationFile', dest='annotationFile', help="optional:A SpliSER formatted Genes.tsv file", required=False)
	parser_process.add_argument('-t', '--threads', dest='threads',nargs='?', default=1, type=int, help="Number of threads with which to run, default: 1", required=False)
	...
	parser_combineShallow.add_argument('-1', '--firstChrom', dest='firstChrom', required=True, help='The name of the first genomic region to be assessed, ie 1, Chr1, Chromosome_1 ,Scaffold1')
